...
function get_release($url,$lang){
    ...
    if(!empty($result[0]["content_release"])){
      return "
            <div>
              <a class='next_button mouse' href='https://agencyfish.com/page/".$result[0]["content_release"]."'>
                  ".get_judul("6",$lang)."             
              </a>
            </div>";
    }  
    ...
    if($result){
      if(count($result)>0){
        if(strtolower($type)=="v"){
          $output = "<div class='y-container'><iframe class='y-video' src='".$result[0]["media_name_url"]."' frameborder='0' allow='autoplay; encrypted-media' allowfullscreen></iframe></div>";
        }else{
