...
class Mod_pinjaman extends CI_Model
{
    ...
    public function dtl_iuran($uid, $tahun)
    {
        ...
        $sintak = "SELECT * FROM (
                SELECT bulan,ifnull(IF(COUNT(1)>1, GROUP_CONCAT(CONCAT(DATE_FORMAT(b.updrec_date,'%m-%d'),'#',c.`nama`)),c.`nama`),'') AS acc,
                ifnull(format(SUM(b.`cash_in`),0,'de_DE'),'') AS total_in,CAST(CONCAT('$th',IF(a.id<10,CONCAT('0',a.id),a.id)) AS INT) AS  id,ifnull(description,'') as desk
                FROM tbl_bulan a LEFT JOIN tbl_transaksi b ON a.`bulan`=MONTHNAME(b.updrec_date)                 
                AND fk_uid=$uid $thn AND fk_mid=2  AND fk_mid=2 LEFT JOIN tbl_user c 
                ON b.`acc_by`=c.`uid` GROUP BY bulan 
                UNION ALL
                SELECT 'total','',ifnull(format(SUM(b.`cash_in`),0,'de_DE'),'') AS total_in,CAST(CONCAT('2022',999) AS INT) AS  id,'' as desk
                FROM tbl_transaksi b WHERE fk_uid=$uid $thn AND fk_mid=2  AND fk_mid=2
                ) abc
                ORDER BY id";
        return each_query($this->db->query($sintak));
    }
