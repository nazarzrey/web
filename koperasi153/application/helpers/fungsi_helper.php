...
if (!function_exists('hide_inet')) {
	function hide_inet()
	{
		$sv = $_SERVER['SERVER_ADDR'];
		if ($sv == "localhost" || $sv == "127.0.0.1" || substr_count($sv, "192") == 1 || $sv == "::1") {
			return " show ";
		} else {
			return " hide ";
		}
...
if (!function_exists('menu_admin')) {
	function menu_admin($menu, $tipe, $title)
	{
		if ($menu == "menu") {
			echo $tipe . "X";
			if (strpos($tipe, "admin") !== false || strpos($tipe, "super") !== false) {
				return " ";
			} else {
				return " hide ";
			}
