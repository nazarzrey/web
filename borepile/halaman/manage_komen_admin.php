	...
	?>
		...
		if (isset($_GET["action"])) {
			...
			function ready_data($field)
			{
				...
				if (isset($_POST[$field])) {
					...
					$adelia = "SELECT $field FROM pu_komentar where id_halaman='$data_dtl2' and id_komentar='$data_idk'";
			...
			function ready_data_berkas($field)
			{
				...
				123456789012345678*/
				$field = trim(htmlentities(mysqli_real_escape_string($field)));
				$data_dtl = substr(trim(htmlentities(mysqli_real_escape_string(str_replace('XX', '', $data_dtl2)))), 3, 15);
				if (isset($_POST[$field])) {
					$hasil_data = $_POST[$field];
				} else {
					$adelia = "SELECT $field FROM pu_pengaduan_berkas WHERE SUBSTR(id_pengaduan,1,15)='$data_dtl'";
					if (mysqli_query($adelia)) {
						$hsl_data = mysqli_fetch_array(mysqli_query($adelia));
					} else {
						$hsl_data = " ";
		...
		?>
				...
				<table>
					...
					<tr>
						...
						<td>
							...
							?>
							<a href="<?php
										echo $Redirected . $page_num; ?>"><input type="button" class="input wx150 button4" style="margin:2px;" value='Kembali' readonly /></a>
						</td>
						</td>
				</table>
			</form>
			...
			if (isset($_POST["simpan"])) {
				$table 			= "pu_komentar";
				if (delete_data_func($table, 'id_komentar', $data_idk, $Redirected) == "Y") {
					$Msg  = "Y";
		...
		<?php
			...
			$query = "
			select * from pu_komentar where aktif='X' order by status ASC, upd_rec desc
			...
			while ($hsl_list_data = mysqli_fetch_array($cek_list_data)) {
				...
				$frm_baca = "<a href='$Redirected$page_num&action=hapus&data_dtl=$hsl_list_data[id_halaman]&idk=$hsl_list_data[id_komentar]' class='button3' style='padding:3px;border-radius:50px'> detail </a>";
