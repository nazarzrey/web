...
$query     = "SELECT a.id_user,a.user_name,a.nama,a.email,a.user_grup,a.aktif,COUNT(DISTINCT(b.id_user)) AS total
			FROM master_user a LEFT OUTER JOIN user_log b
		...
		<div id="list_result" style="max-height:450px;overflow:auto;width:100%;">
			...
			foreach ($cek_list_data as $key => $hsl_list_data) {
				if ($hsl_list_data["aktif"] == "Y" and $hsl_list_data["total"] != 0) {
					...
					$url_action = "<a href='$Redirected&action=edit&data_dtl=$hsl_list_data[id_user]' class='edit'></a><a href='$Redirected&action=reset&data_dtl=$hsl_list_data[id_user]' class='reset'>";
				} elseif ($hsl_list_data["aktif"] == "Y" and $hsl_list_data["total"] == 0) {
					$style_aktif = "style='color:#006ab4';";
					$url_action = "<a href='$Redirected&action=edit&data_dtl=$hsl_list_data[id_user]' class='edit'></a> <a href='$Redirected&action=delete&data_dtl=$hsl_list_data[id_user]' class='delete'></a><a href='$Redirected&action=reset&data_dtl=$hsl_list_data[id_user]' class='reset'></a></a>";
				...
				echo "<tr class='tr'><td class='right'  $style_aktif>$x</td><td class='right'  $style_aktif>" . $hsl_list_data["user_name"] . "</td><td  $style_aktif>" . $hsl_list_data["nama"] . "</td><td $style_aktif>" . $hsl_list_data["user_grup"] . "</td><td  $style_aktif>$hsl_list_data[aktif]</td><td>$hsl_list_data[total]</td><td>$url_action</td></tr>";
				$x++;
			}
			?>
		</div>
	<?php
		...
		function cek_post_user($data_post, $field)
		{
			if ($_GET["action"] == "add") {
				...
				global $data_dtl, $table, $cek_id, $db;
		...
		if (isset($_POST["simpan"])) {
			...
			if (substr($cek_post, 0, 1) == "N") {
				...
				if ($_GET["action"] == "add") {
					...
					$cek_data_exists = $db->select($xa);
					if (count($cek_data_exists) == 1) {
						$xx = "select 1 from user_log where $cek_id='$_GET[data_dtl]'";
