...
?>
	<div class="new_info_left">
		...
		<div style="overflow:auto;height:450px;width:104.5%;margin-top:25px;">
			<?php
			$inf_query  = $db->select("select * from info where untuk='$Id_user' order by tanggal desc");
			while ($inf_result = count($inf_query)) {
				if ($inf_result[4] != "N") {
			...
			<div class='info_list $info_backgr' style='width:46%;height:120px;;'>
				...
				$pengirim = count($db->select("select nama from master_user where id_user='$inf_result[2]'"));
	...
	<div class="new_info_right">
		...
		<div style="overflow:auto;height:500px;width:105%;margin-top:25px;">
			<?php
			$inf_query  = $db->select("select *  FROM info  WHERE dari='$Id_user'  GROUP BY isi ORDER BY tanggal DESC");
	...
	if (isset($_GET["dtl_info"])) {
		$inf_query  = $db->select("select c.*,a.nama FROM info c,MASTER_USER a WHERE c.dari=a.id_user AND c.id_info='$_GET[dtl_info]'");
		$inf_rows = count($inf_query);
		if ($inf_rows > 0) {
		...
		$inf_query  = $db->select("select c.*,a.nama FROM info c,MASTER_USER a WHERE c.dari=a.id_user AND c.id_info='$_GET[dtl_info2]'");
		$inf_rows = count($inf_query);
		if ($inf_rows > 0) {
			$inf_resutl = mysql_fetch_array($inf_query);
			...
			<table border='0' width="100%">
				...
				<tr>
					...
					<td>
						...
						<?php if ($Ms_grup != "DC") {
							echo '<span class="notif_2">* Pemilihan pengirim, (DC = ALL USER DC), (Support DC = hanya ALL Support DC dan dc user tidak terima infonya)</span>';
						} ?>
					</td>
				</tr>
			</table>
...
if (isset($_POST['save'])) {
	...
	if ($Ms_grup == "DC" and $untuk == "Support DC") {
		...
		if ($untuk != "DC" and $untuk != "IT" and $untuk != "LOGHO" and $untuk != "Support DC") {
			$cek_nama_send = $db->select("select upper(nama) from master_user where nama='" . strtoupper($untuk) . "'");
			if (count($cek_nama_send) == 0) {
				$cek_dc_send = $db->select("select distinct(upper(kode_dc)) from master_user where kode_dc='" . substr(strtoupper($untuk), 0, 4) . "'");
				if (count($cek_dc_send) == 0) {
		...
		$tgl   			 = date('Y-m-d');
		$dari  			 = $Id_user;
		$cek_send  		 = $db->select("select distinct(group_user) as grup from master_user union ALL select distinct(kode_dc) as grup from master_user where  kode_dc<>'HO'");
		...
		if ($send_grup == 'Y') {
			if ($sdc == "YES") {
				$sintak = $db->select("select id_user from master_user where kode_dc='" . $untuk . "'");
			} else {
				if ($untuk == "Support DC") {
					$sintak = $db->select("select id_user from master_user where group_user='DC' and type_user='support'");
				} elseif ($untuk == "DC" and $Ms_grup == "DC") {
					$sintak = $db->select("select id_user from master_user where kode_dc='" . $Ms_dc . "'");
				} elseif ($untuk == "DC" and $Ms_grup != "DC") {
					$sintak = $db->select("select id_user from master_user where group_user='" . $untuk . "'");
				} else {
					$sintak = $db->select("select id_user from master_user where group_user='" . $untuk . "'");
				}
			}
