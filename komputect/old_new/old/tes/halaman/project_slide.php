...
<body style="font-family:Arial, Verdana;background-color:#fff;">
	...
	if(isset($_GET['proyek_dtl'])){
        $Cl_query  = mysql_query("select * from project where produk='$_GET[proyek_dtl]' order by tahun desc,upd_data desc limit $var_dtl")or die (mysql_error());
	}else{
		$Cl_query  = mysql_query("select * from project $proyek_type order by tahun desc,upd_data desc limit $var_dtl")or die (mysql_error());
	}		
			while ($Cl_result = mysql_fetch_array($Cl_query)){
				$Cl_img  = $Cl_result['project_img'];
				$Cl_href = "?halaman=proyek&project_id=".$Cl_result['project_id'];
