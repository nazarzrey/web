<div id="name-list" style='height:auto;padding:10px;'>
  ...
  <form method="post" action="" enctype="multipart/form-data" >
	<?php
	$prd_dtl_query  = mysql_query("select * from product where product_id='$_GET[data_product]'")or die(mysql_error());
	if(mysql_num_rows($prd_dtl_query)==0){
		echo "data not found";
	}else{
		$prd_dtl_result= mysql_fetch_array($prd_dtl_query);
		echo "<input typ='text' name='key_prd_dtl' value='$prd_dtl_result[product_id]' style='display:none;'>";
		echo '<tr><td width="15%">Urut</td><td><input type="text" required="required" name="bld" class="ipro ro" value="'.$prd_dtl_result[0].'" readonly></tr>';
		echo '<tr><td>Build Name</td><td><input type="text" required="required" name="bld" class="ipro ro" value="'.$prd_dtl_result[3].'" readonly></td></tr>';
		echo '<tr><td>Image Proyek</td><td>';
			$img_proj_utama = $prd_dtl_result[5];
					...
					if($cnt >$max_size_upload ){
						echo '<script>alert("Maaf total file melebihi ketentuan yaitu '.$max_size_upload.' \\njadi tidak bisa di upload")</script>';
							...
							foreach($_FILES['berkas2']['tmp_name'] as $key => $tmp_name) {
								...
								$insert_product_dtl = mysql_query("insert into product_detail values ('$_POST[key_prd_dtl]','$Pno','X','$thumb_file','$full_file',now())")or die(mysql_error());
								$Pno++;
							}
						}
					echo "<msg>Image galery for product $_POST[bld] has uploaded</msg>";
