...
<?php 
	$data_dtl = $_GET["data_project"];
	$web_page2 = $web_page."&action=ins_gal&data_project=$data_dtl";
	$data_cntn  = mysql_fetch_array(mysql_query("select build_name,lokasi,tahun,produk from project where project_id='$data_dtl'")) or die(mysql_error());
...
?>
	...
	<hr width="100%" align="center" color="orange" style="float:left;margin-top:3px;">	
		<table border='0' width='100%' class="action_in">
		  <form method="post" action="" enctype="multipart/form-data" >
			<?php
			$dtl_query  = mysql_query("select count(recid),max(urut) from project_detail where project_id='$data_dtl'")or die(mysql_error());
			...
			echo '<tr><td >Project Name </td><td><input type="text" class="ipro ro" name="data_name" value="'.$data_cntn[0].'" readonly ></td></tr>';
			echo '<tr><td >Location</td><td><input type="text" class="ipro ro" value="'.$data_cntn[1].'" readonly "></td></tr>';
			echo '<tr><td >Product - Year</td><td><input type="text" name="total_galery" class="ipro ro" value="'.$data_cntn[3]." - ".$data_cntn[2].'"readonly></td></tr>';
							...
							if($cnt!=0){
								if($cnt >$max_size_upload ){
									...
									foreach($_FILES['berkas2']['tmp_name'] as $key => $tmp_name) {
										...
										$insert_dtl = mysql_query("insert into project_detail(project_id,urut,recid,small_img,big_img,upd_data,id_galery)	values ('$data_dtl','$Pno','X','$thumb_file','$full_file',now(),'$id_galery')")or die(mysql_error());
									$Pno++;
									}
	...
	<?php 
		...
		$master = " from project_detail where project_id='$data_dtl'";
		...
		if(isset($_POST["upd_galery"])){
			if(isset($_POST['delete_galery'])){
				...
				foreach ($delete as $id => $val) {
					if(isset($val)){
						$hapus_master = "from project_detail where id_galery='$id'";
