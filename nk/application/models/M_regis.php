...
class M_regis extends CI_Model{
	...
	function get_all_siswa(){
		$hsl=$this->db->query("SELECT * FROM tbl_daftar where csiswa_sync='0'");
		return $hsl;
	}

	function simpan_siswa($jenis,$jalur,$no,$nama,$jenkel,$tmp_lahir,$tgl_lahir,$alamat,$rt,$rw,$desa,$kec,$kab,$pos,$tmp_tgl,$hp,$tlp,$email,$tinggi,$berat,$jarak,$waktu,$sdr,$photo,$ayh,$thn_ayh,$pdk_ayh,$job_ayh,$gaji_ayh,$no_ayh,$ibu,$thn_ibu,$pdk_ibu,$job_ibu,$gaji_ibu,$no_ibu,$wali,$thn_wali,$pdk_wali,$job_wali,$gaji_wali,$no_wali,$nisn){
		$hsl=$this->db->query("INSERT INTO tbl_daftar (csiswa_jenis,csiswa_jalur,csiswa_no,csiswa_nama,csiswa_jenkel,csiswa_tmp_lahir,csiswa_tgl_lahir,csiswa_alamat,csiswa_rt,csiswa_rw,csiswa_desa,csiswa_kec,csiswa_kab,csiswa_pos,csiswa_tmp_tgl,csiswa_hp,csiswa_tlp,csiswa_email,csiswa_tinggi,csiswa_berat,csiswa_jarak,csiswa_waktu,csiswa_sdr,csiswa_photo,csiswa_ayh,csiswa_thn_ayh,csiswa_pdk_ayh,csiswa_job_ayh,csiswa_no_ayh,csiswa_gaji_ayh,csiswa_ibu,csiswa_thn_ibu,csiswa_pdk_ibu,csiswa_job_ibu,csiswa_gaji_ibu,csiswa_no_ibu,csiswa_wali,csiswa_thn_wali,csiswa_pdk_wali,csiswa_job_wali,csiswa_gaji_wali,csiswa_no_wali,csiswa_nisn) VALUES ('$jenis','$jalur','$no','$nama','$jenkel','$tmp_lahir','$tgl_lahir','$alamat','$rt','$rw','$desa','$kec','$kab','$pos','$tmp_tgl','$hp','$tlp','$email','$tinggi','$berat','$jarak','$waktu','$sdr','$photo','$ayh','$thn_ayh','$pdk_ayh','$job_ayh','$gaji_ayh','$no_ayh','$ibu','$thn_ibu','$pdk_ibu','$job_ibu','$gaji_ibu','$no_ibu','$wali','$thn_wali','$pdk_wali','$job_wali','$gaji_wali','$no_wali','$nisn')");
		return $hsl;
	}
	function simpan_siswa_tanpa_img($jenis,$jalur,$no,$nama,$jenkel,$tmp_lahir,$tgl_lahir,$alamat,$rt,$rw,$desa,$kec,$kab,$pos,$tmp_tgl,$hp,$tlp,$email,$tinggi,$berat,$jarak,$waktu,$sdr,$ayh,$thn_ayh,$pdk_ayh,$job_ayh,$gaji_ayh,$no_ayh,$ibu,$thn_ibu,$pdk_ibu,$job_ibu,$gaji_ibu,$no_ibu,$wali,$thn_wali,$pdk_wali,$job_wali,$gaji_wali,$no_wali,$nisn){
	...
	function sync_siswa($kode,$kelas){
		$sql = "INSERT INTO  tbl_siswa (siswa_nis,siswa_nisn,siswa_nama,siswa_password,siswa_jenkel,siswa_tempat_lahir,siswa_tanggal_lahir,siswa_ayah,siswa_ibu,siswa_alamat,siswa_kelas_id,siswa_tipe )
				SELECT csiswa_no,csiswa_nisn,csiswa_nama,DATE_FORMAT(csiswa_tgl_lahir,'%d%m%y') AS pwd,csiswa_jenkel,csiswa_tmp_lahir,csiswa_tgl_lahir,csiswa_ayh,csiswa_ibu,CONCAT(csiswa_alamat,' RT ',csiswa_rt,' / RW ',csiswa_rw) AS alamat,'$kelas', LOWER(LEFT(csiswa_jenis,1)) AS jenis FROM tbl_daftar
				where csiswa_id='$kode'";
		if($this->db->query($sql)){
			$sintak  = "update tbl_daftar set csiswa_sync='1' where csiswa_id='$kode'";
			$hsl = $this->db->query($sintak);
		};
		return $hsl;
	}
	function siswa(){
