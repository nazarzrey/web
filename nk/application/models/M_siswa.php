...
class M_siswa extends CI_Model{
	...
	function get_all_siswa(){
		$hsl=$this->db->query("SELECT tbl_siswa.*,kelas_nama,IFNULL(SUM(d.tab_in - d.tab_out),0) AS tabungan FROM tbl_siswa JOIN tbl_kelas ON siswa_kelas_id=kelas_id
							LEFT JOIN tbl_tabungan d ON tbl_siswa.`siswa_id`=d.`siswa_fk_id`
							GROUP BY siswa_id");
		return $hsl;
	}
